...
bool verify(const float* ref, const float* out, int size, float tol = 1e-3f) {
    for (int i = 0; i < size; i++)
        if (fabsf(ref[i] - out[i]) > tol) {
...
int main(int argc, char** argv) {
    ...
    printf("Initializing Naive GEMM: M=%d, N=%d, K=%d\n", M, N, K);
    ...
    // FIX: fixed seed for reproducibility across all kernel versions
    srand(42);
